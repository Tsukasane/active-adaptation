...
class tracking_end_effector(Reward):
    def __init__(self, env, weight: float, enabled: bool = True, sigma: float = 0.1, 
                 upper_body_names: str=".*", upper_body_err_weight: float=1.0,
                 lower_body_names: str=".*", lower_body_err_weight: float=1.5):
        super().__init__(env, weight, enabled)
    ...
    def compute(self) -> torch.Tensor:
        timestep = self.env.episode_length_buf.unsqueeze(1) - 1
        ref_keypoints = self.env.command_manager.ref_keypoints[timestep].squeeze(1).reshape(self.num_envs, -1, 3)    # [num_envs, num_keypoints, 3]
