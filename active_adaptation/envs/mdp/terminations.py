...
class cum_error_root(Termination):
...
class cum_error_vel(Termination):
    ...
    def update(self):
        cum_error = self.command_manager._cum_error_vel
        error_exceeded = (cum_error > self.thres).any(-1, True)
        self.error_exceeded_count[error_exceeded] += 1
        self.error_exceeded_count[~error_exceeded] = 0
...
class cum_error_qpos(Termination):
    def __init__(self, env, thres: float = 0.85, min_steps: int = 50):
        super().__init__(env)
        self.thres = torch.tensor(thres, device=self.env.device)
        self.min_steps = min_steps
    ...
    def update(self):
        cum_error = self.command_manager._cum_error_qpos
        error_exceeded = (cum_error > self.thres).any(-1, True)
        self.error_exceeded_count[error_exceeded] += 1
        self.error_exceeded_count[~error_exceeded] = 0
...
class cum_error_kp(Termination):
    def __init__(self, env, thres: float = 0.85, min_steps: int = 50):
        super().__init__(env)
        self.thres = torch.tensor(thres, device=self.env.device)
        self.min_steps = min_steps
    ...
    def update(self):
        cum_error = self.command_manager._cum_error_keypoint
        error_exceeded = (cum_error > self.thres).any(-1, True)
        self.error_exceeded_count[error_exceeded] += 1
        self.error_exceeded_count[~error_exceeded] = 0
