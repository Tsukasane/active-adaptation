...
class MuJoCoMocapViewer:
    def __init__(
        ...
        self.box_subscriber = ZMQSubscriber(PORTS['box_pose'])
        ...
        box_joint_qpos_adr = self.model.jnt_qposadr[box_joint_id]
        self.box_joint_qpos_adr = box_joint_qpos_adr
        ...
        print("MuJoCo Mocap Viewer (Box) initialized, waiting for data...")
    ...
    def zmq_communication_loop(self):
        ...
        while self.running:
            ...
            box_msg = self.box_subscriber.receive_pose()
            if box_msg:
                box_qpos = np.concatenate([box_msg.position, box_msg.quaternion])
                self.data.qpos[self.box_joint_qpos_adr: self.box_joint_qpos_adr + 7] = box_qpos
